...
for (Alg, desc, refs, is_W) in [
        ...
        (:Tsit5DA, "A 12-stage order 5(4) hybrid explicit/linear-implicit method for semi-explicit index-1 DAEs.\nDifferential variables are treated explicitly (like Tsit5), algebraic variables use Rosenbrock-type\nlinear-implicit stages. Only the small algebraic Jacobian block needs factorization.\nFor pure ODEs (no algebraic constraints), reduces to an explicit Runge-Kutta method.", "- Steinebach G., Rodas6P and Tsit5DA - two new Rosenbrock-type methods for DAEs.\n  arXiv:2511.21252, 2025.", false),
    ]
    @eval begin
