...
if TEST_GROUP == "ALL" || TEST_GROUP == "Core"
    ...
    @time @safetestset "_resolve_rng prefers prob.seed over TaskLocalRNG" begin
        # Regression test for the weak-convergence PL1WM / W2Ito1 failures:
        # when the SciMLBase v3 ensemble layer passes `rng=Random.default_rng()`
        # (TaskLocalRNG) and `prob.seed` is non-zero (typical after
        # `remake(prob, seed=s)` inside a `prob_func`), the integrator must
        # honor `prob.seed` rather than consuming a seed from the TaskLocalRNG.
        # Otherwise two algorithms that differ only in how much randomness they
        # draw per step (e.g. PL1WM vs PL1WMA for additive noise, where PL1WM
        # needs an extra Z process) will see unrelated noise streams from the
        # same ensemble seed assignment.
        using StochasticDiffEqCore
        using Random
        using DiffEqBase
        using Test
